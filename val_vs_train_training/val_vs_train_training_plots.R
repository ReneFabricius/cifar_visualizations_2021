...
net_results_c10 <- read.csv(
    "../data/data_train_val_half_c10/net_metrics.csv", stringsAsFactors = TRUE)
ens_results_c10 <- read.csv(
    "../data/data_train_val_half_c10/ensemble_metrics.csv", stringsAsFactors = TRUE)
...
for (comb_id in unique(ens_results_c10$combination_id))
{
    ...
    for (met_i in seq_along(metrics))
    {
        ...
        plot_name <- paste0(
            "val_vs_train_training/CIFAR-10_",
            metrics[met_i], "_",
...
for (met_i in seq_along(metrics))
{
    ...
    plot_name <- paste0("val_vs_train_training/CIFAR-10_", metrics[met_i], "_test.pdf")
    ggsave(filename = plot_name, plot = col_plot, device = cairo_pdf)
}

met_comp_c10 <- ens_results_c10 %>% group_by(combination_id, combining_method, coupling_method, train_set) %>%
...
for (met_i in seq_along(metrics))
{
    ...
    plot_name <- paste0("val_vs_train_training/CIFAR-10_", metrics[met_i], "_diff.pdf")
...
for (comb_id in unique(ens_results_c100$combination_id))
{
    ...
    for (met_i in seq_along(metrics))
    {
        ...
        plot_name <- paste0("val_vs_train_training/CIFAR-100_", metrics[met_i], "_", paste(comb_nets, collapse = "_"), ".pdf")
        ggsave(filename = plot_name, plot = box_plot, device = cairo_pdf)
    }
}
...
for (met_i in seq_along(metrics))
{
    ...
    plot_name <- paste0("val_vs_train_training/CIFAR-100_", metrics[met_i], "_test.pdf")
    ggsave(filename = plot_name, plot = col_plot, device = cairo_pdf)
}

met_comp_c100 <- ens_results_c100 %>% group_by(combination_id, combining_method, coupling_method, train_set) %>%
                    summarise(mean_accuracy = mean(accuracy), mean_nll = mean(nll), mean_ece = mean(ece)) %>%
                    pivot_wider(names_from = train_set, values_from = c(mean_accuracy, mean_nll, mean_ece)) %>%
                    mutate(
                        accuracy_vt_sub_tt = mean_accuracy_vt - mean_accuracy_tt,
...
for (met_i in seq_along(metrics))
{
    ...
    plot_name <- paste0("val_vs_train_training/CIFAR-100_", metrics[met_i], "_diff.pdf")
    ggsave(filename = plot_name, plot = dens_plt, device = cairo_pdf)
}