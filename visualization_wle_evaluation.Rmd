---
title: "WeightedLinearEnsemble evaluation"
output:
...
```{r}
base_dir <- "D:/skola/1/weighted_ensembles/tests/test_cifar_2021/data/data_tv_5000_c100/0/"
net_df <- read.csv(file.path(base_dir, "net_metrics.csv"))
ens_df_cal <- read.csv(file.path(base_dir, "ens_cal_metrics_temp.csv"))
ens_df_pwc <- read.csv(file.path(base_dir, "ens_pwc_metrics_temp.csv"))

net_long <- pivot_longer(net_df, cols=c("accuracy", "nll", "ece"), 
  names_to="metric", values_to="value")
ens_cal_long <- pivot_longer(ens_df_cal, cols=c("accuracy", "nll", "ece"),
  names_to="metric", values_to="value")
ens_pwc_long <- pivot_longer(ens_df_pwc, cols=c("accuracy", "nll", "ece"),
  names_to="metric", values_to="value")

networks <- net_df$network

comb_stats_df = data.frame(matrix(ncol=14, nrow=0, 
...
for (sss in unique(ens_df_cal$combination_size))
{
                      ...
                      pull(combination_id)))
  {
    ...
    comb_stats_df[nrow(comb_stats_df) + 1, ] = c(sss, ssi,
                      ...
                      pull(combination_id)))
  {
    ...
    acc_plot <- ggplot() +
      ...
      (
        geom_hpline(
          data=cur_ens_pwc,
          mapping=aes(x=combining_method, y=accuracy,
                      colour2=coupling_method),
          size=0.8, width=0.11,
          position=position_dodge(width=0.65)) %>%
        rename_geom_aes(new_aes = c("colour" = "colour2"))
      ) +
      scale_colour_brewer(aesthetics="colour1", palette=1,
        name="network", type="qual") +
      scale_colour_brewer(aesthetics="colour2", palette=2,
      ...
      acc_plot <- acc_plot +
        geom_rect(
          ...
          mapping=aes(
            xmin=0.5,
            xmax=length(x_limits) + 0.5,
            ymin=all_y_lim[1], ymax=all_y_lim[2]), fill="green",
            alpha=0.3, color=NA)
    }
    ...
    nll_plot <- ggplot() +
      ...
      (
        geom_hpline(
          data=cur_ens_pwc,
          mapping=aes(x=combining_method, y=nll,
                      colour2=coupling_method),
          size=0.8, width=0.11,
          position=position_dodge(width=0.65)) %>%
        rename_geom_aes(new_aes = c("colour" = "colour2"))
    ...
    ece_plot <- ggplot() +
      ...
      (
        geom_hpline(
          data=cur_ens_pwc,
          mapping=aes(x=combining_method, y=ece,
                      colour2=coupling_method),
          size=0.8, width=0.11,
          position=position_dodge(width=0.65)) %>%
        rename_geom_aes(new_aes = c("colour" = "colour2"))
...
xax <- c("err_incons",
  "nll_var", "nll_min", "nll_max", "nll_avg",
  "acc_var", "acc_min", "acc_max", "acc_avg",
  "ece_var", "ece_min", "ece_max", "ece_avg")
  ...
  for (xa in xax)
  {
    for (ya in yax)
    {
      cur_plot <- ggplot() +
        ...
        ggtitle(sprintf("Accuracy improvement of ensemble over
...
imp_avg_plot <- ggplot() +
  geom_boxplot(data=ens_pwc_plt_df %>% filter(coupling_method != "sbt"), mapping=aes(x=coupling_method, y=acc_imp_avg)) +
...
```{r, fig.width=9, fig.height=10}
imp_max_plot <- ggplot() +
  geom_boxplot(data=ens_pwc_plt_df %>% filter(coupling_method != "sbt"), mapping=aes(x=coupling_method, y=acc_imp_max)) +
