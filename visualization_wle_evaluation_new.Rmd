...
```{r}
base_dir <- "D:/skola/1/weighted_ensembles/tests/test_cifar_2021/data/data_tv_5000_c100/0/new"
...
{
  imp_avg_plot <- ggplot() +
    geom_boxplot(data = ens_pwc_plt_df %>% filter(coupling_method != "sbt" & combination_size == sss), mapping = aes(x = coupling_method, y = acc_imp_avg)) +
  ...
  imp_max_plot <- ggplot() +
    geom_boxplot(data = ens_pwc_plt_df %>% filter(coupling_method != "sbt" & combination_size == sss), mapping = aes(x = coupling_method, y = acc_imp_max)) +
