...
box_acc
```
Training on the same training data as the neural networks were trained on seems to provide better accuracy compared to training on separate validation set. This difference is bigger for bc coupling method, than for m1 and m2.
...
```
Outputs of each coupling method has statistically higher accuracy when LDA is trained on training set of neural networks.
