...
plot_plots <- function(base_dir, eval_dir, cifar)
{
    ...
    limits_single <- list(
        "acc" = list("10" = c(0.0, 0.03), "100" = c(0, 0.065)),
    ...
    for (met_i in seq_along(metrics))
    {
        ...
        metric_plot <- ens_df_pwc %>%
                    ...
                    geom_boxplot(
                        ...
                        mapping = aes_string(
                            ...
                            color = shQuote("bez regularizácie")),
                        width = C_range,
                        alpha = 0.5) +
                    geom_boxplot(
                        mapping = aes_string(
        ...
        file_name <- paste0("grad_reg_c", cifar, "_", met, "_imp_over_", improvement_over, ".pdf")
        ...
        metric_plot <- ens_df_pwc %>% filter(str_split(method, pattern = fixed(" + "), simplify = TRUE)[, 1] == "grad_m2") %>%
                    ...
                    geom_boxplot(
                        data = ens_eval_df_pwc %>% 
                            filter(str_split(method, pattern = fixed(" + "), simplify = TRUE)[, 1] == "grad_m2"),
                        mapping = aes_string(
                            y = paste0(met, "_imp_", improvement_over),
                            color = shQuote("bez regularizácie")),
                        width = C_range,
        ...
        file_name <- paste0("print_grad_reg_c", cifar, "_", met, "_imp_over_", improvement_over, ".pdf")
