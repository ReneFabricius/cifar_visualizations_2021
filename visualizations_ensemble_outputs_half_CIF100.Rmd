...
Otherwise similar results to those for CIFAR 10. Ensembles trained on validation set have smoother histograms of correct class probability.
...
for (cls in 1:classes)
{
  ...
  plot_cls <-  ggplot(cur_class_Rs, aes(x = class2, y = class1)) + 
    ...
    scale_fill_binned(type="viridis", limits=c(-0.9, 0.9), breaks=seq(-.8, .8, .4), name="vt minus tt") +
    scale_y_discrete(limits=rev, breaks=seq(0, classes, 10)) +
    scale_x_discrete(breaks=seq(0, classes, 10)) +
